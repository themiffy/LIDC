...
test_quantity = 20 #get rid too!
image_quantity = 25 #How many images are in set. GET RID OF IT LATER
img_width = 512
img_height = 512
img_channels = 3
start_neurons = 16
...
X_train = np.zeros((len(trainFileNames), img_width, img_height, img_channels), dtype=np.uint8)
...
for file in glob.glob(train_path + 'images/*'):
    ...
    img = imread(file)[:,:,:img_channels] # arrays are probably ZEROS!?
    #resize if needed
    img = resize(img, (img_height, img_width), mode='constant', preserve_range=True)
    X_train[n] = img
 
    mask = imread(file[0:36]+ 'masks/rtop/'+ file[43:53]+ '_Rhigh.ome.tiff')
    mask = resize(mask, (img_height, img_width), mode='constant', preserve_range=True)
    Y_train[n] = mask
    n+=1
 
# test images
X_test = np.zeros((len(testFileNames), img_height, img_width, img_channels), dtype=np.uint8)
n = 0
...
sen = 0.2 # 0.5 by default, but not working. Makes everything 0!
 
preds_train_t = (preds_train > sen).astype(np.uint8)
preds_val_t = (preds_val > sen).astype(np.uint8)
preds_test_t = (preds_test > sen).astype(np.uint8)
...
plt.show()
imshow(np.squeeze(Y_train[int(Y_train.shape[0]*0.9):][ix-1]))
plt.show()
imshow(np.squeeze(preds_val_t[ix-1]))
plt.show()